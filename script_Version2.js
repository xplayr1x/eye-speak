...
const wordsArray = [
  ...
  "go", "come", "stop", "run", "eat", "drink",
  "now", "later", "today", "tomorrow", "yes", "no",
  "happy", "sad", "angry", "tired", "good", "bad",
  "really", "very", "so", "too", "much", "a lot",
  "hello", "bye", "please", "thanks", "help", "sorry",
  "home", "work", "fuck", "food", "water", "music",
  "play", "look", "wait", "okay", "great", "wow",  "ugh", "oops", "amazing", "cool", "fantastic",
];
...
recalibrateBtn.addEventListener('click', ()=>{
  ...
  setTimeout(()=> recalibrateBtn.textContent="Recalibrate",2000);
});
...
faceMesh.onResults((results)=>{
  ...
  if(now-gazeDelay>150){
    if(ratioX<0.35) updateActiveWord((activeIndex-1+words.length)%words.length);
  ...
  if(ear<EAR_THRESHOLD){
    ...
    if(earR<EAR_THRESHOLD && earL>=EAR_THRESHOLD && now-lastBlinkTime>BLINK_COOLDOWN){
      const wordsArr = sentenceSpan.textContent.trim().split(" ");
      wordsArr.pop();
      sentenceSpan.textContent = wordsArr.join(" ") + (wordsArr.length ? " " : "");
      lastBlinkTime=now;
    }
    ...
    // --- Short blink (both eyes): select word ---
